...
context('Import Products', () => {
    it('round price - percent - csv - file  - new job', () => {
        ...
        cy.get('#conditions__1__children').find('a').should('have.value','').as('priceSku')
        cy.get('@priceSku').click()
        cy.get('[name="rule[conditions][1--1][value]"]').invoke('val', 'TST-Conf-Simp-S-Green').trigger('change',{force:true})
