...
context('Import Products Custom logic Status Enabled Copy Without Parent 32',{ retries: 0 }, () => {
    it('custom logic - status - enabled - copy - without parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - status - enabled  - Without parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_status_enabled_without_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'status-two').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        cy.get('table',{timeout:40000}).contains('status-two-S-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Enabled')
            })
