...
context('Import Products Custom logic Visibility Not visible Copy Without Parent 20',{ retries: 0 }, () => {
    it('custom logic - visibility - not visible - copy - without parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - visibility - not visible  - Without parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_visibility_not-visible_without_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'visibility-two').trigger('change',{force:true})
        ...
        cy.get('table',{timeout:40000}).contains('visibility-two-S-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Not Visible Individually')
            })
            cy.get('table',{timeout:40000}).contains('visibility-two-S-Purple').parentsUntil('tbody')
            .within(function () {
                cy.contains('Not Visible Individually')
            })
            cy.get('table',{timeout:40000}).contains('visibility-two-M-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Not Visible Individually')
            })
            cy.get('table',{timeout:40000}).contains('visibility-two-M-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Not Visible Individually')
            })
        ...
        //delete 'visibility-two' products
