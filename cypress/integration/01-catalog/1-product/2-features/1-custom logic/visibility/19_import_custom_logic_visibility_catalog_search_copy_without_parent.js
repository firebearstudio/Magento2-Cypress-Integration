...
context('Import Products Custom logic Visibility Catalog/Search Copy Without Parent 19',{ retries: 0 }, () => {
    it('custom logic - visibility - catalog/search - copy - without parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - visibility - copy - catalog/search  - Without parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_visibility_catalog_search_without_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'visibility-one').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        cy.get('table',{timeout:40000}).contains('visibility-one-S-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog, Search')
            })
        cy.get('table',{timeout:40000}).contains('visibility-one-S-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-one-S-Purple').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-one-M-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-one-M-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog, Search')
            })
        ...
        //delete 'visibility-one' products
