...
context('Import Products Remove Images 3',{ retries: 3 }, () => {
    it('remove images - add update -  csv - file - new job', () => {
        ...
        cy.get('[data-index="remove_images"]').find('.admin__actions-switch-label').as('removeImage')
        cy.get('@removeImage').click()
        cy.get('[data-index="remove_images_dir"]').find('.admin__actions-switch-label').as('removeImageFromDirectory')
        cy.get('@removeImageFromDirectory').click()
        ...
        //check that images were deleted from admin panel
