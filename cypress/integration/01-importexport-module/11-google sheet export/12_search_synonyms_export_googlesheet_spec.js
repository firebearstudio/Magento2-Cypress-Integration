...
context('Export Search Synonyms GoogleSheet 1',() => {
    it('export - googlesheet - new job', () => {
        ...
        cy.get('.general_title ').find('input')
            .type('Search Synonyms Export - googlesheet')
            .should('have.value', 'Search Synonyms Export - googlesheet')

        //specify Export Settings section
        cy.get('.settings_entity').find('select').as('settingsEntity')
        cy.get('@settingsEntity').select('search_synonyms',{force:true})
        ...
        cy.get('[name="export_source_google_sheet_id"]').type('1109053214')
