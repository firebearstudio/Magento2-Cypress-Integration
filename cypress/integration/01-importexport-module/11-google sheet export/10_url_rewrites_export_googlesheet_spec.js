...
context('Export Url Rewrites GoogleSheet 10',() => {
    it('export - googlesheet - new job', () => {
        ...
        cy.get('.general_title ').find('input')
            .type('Url Rewrites Export - googlesheet')
            .should('have.value', 'Url Rewrites Export - googlesheet')

        //specify Export Settings section
        cy.get('.settings_entity').find('select').as('settingsEntity')
        cy.get('@settingsEntity').select('url_rewrite',{force:true})
        ...
        cy.get('[name="export_source_google_sheet_id"]').type('196318278')
