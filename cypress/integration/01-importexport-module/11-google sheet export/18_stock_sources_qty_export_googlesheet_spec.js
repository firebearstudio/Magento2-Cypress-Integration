...
context('Export Stock Sources Qty GoogleSheet 1',() => {
    it('export - googlesheet - new job', () => {
        ...
        cy.get('.general_title ').find('input')
            .type('Stock Sources Qty Export - googlesheet')
            .should('have.value', 'Stock Sources Qty Export - googlesheet')

        //specify Export Settings section
        cy.get('.settings_entity').find('select').as('settingsEntity')
        cy.get('@settingsEntity').select('stock_sources_qty',{force:true})
        ...
        cy.get('[name="export_source_google_sheet_id"]').type('567797374')
