...
context('Import Custom Options Type Checkbox 6', () => {
    it('import custom options - type checkbox - new job', () => {
        //login
        ...
        cy.generalImportSection('Product Import - custom options - type checkbox')
        ...
        cy.fileSource('pub/media/importexport/test/custom-options-type-checkbox.csv',{force:true})
        ...
        cy.get('[name="sku"]').invoke('val', 'custom-options-type-check').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        //check the custom options values
        cy.get('.admin__data-grid-outer-wrap').contains('custom-options-type-checkbox',{timeout: 10000})
        cy.get('.admin__data-grid-outer-wrap').contains('custom-options-type-checkbox').click({force:true});
        cy.get('[data-index="custom_options"]',{timeout: 20000}).find('.fieldset-wrapper-title').click({force:true});
        cy.get('[data-index="options"]').find('[name="product[options][0][title]"]').should('have.value','Custom6')
        cy.get('[data-index="options"]').find('[data-index="type"]').contains('Checkbox')
