...
context('Import Custom Options Type Radio 5', () => {
    it('import custom options - type radio - new job', () => {
        //login
        ...
        cy.generalImportSection('Product Import - custom options - type radio')
        ...
        cy.fileSource('pub/media/importexport/test/custom-options-type-radio.csv',{force:true})
        ...
        cy.get('[name="sku"]').invoke('val', 'custom-options-type-radi').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        //check the custom options values
        cy.get('.admin__data-grid-outer-wrap').contains('custom-options-type-radio',{timeout: 10000})
        cy.get('.admin__data-grid-outer-wrap').contains('custom-options-type-radio').click({force:true});
        cy.get('[data-index="custom_options"]',{timeout: 20000}).find('.fieldset-wrapper-title').click({force:true});
        cy.get('[data-index="options"]').find('[name="product[options][0][title]"]').should('have.value','Custom5')
        cy.get('[data-index="options"]').find('[data-index="type"]').contains('Radio Buttons')
        cy.get('[data-index="options"]').find('[data-index="values"]').find('tbody').find('tr').should('have.length', 2)
        cy.get('[data-index="options"]').find('[name="product[options][0][values][0][title]"]').should('have.value','g')
        ...
        cy.get('[data-index="options"]').find('[name="product[options][0][values][1][title]"]').should('have.value','h')
