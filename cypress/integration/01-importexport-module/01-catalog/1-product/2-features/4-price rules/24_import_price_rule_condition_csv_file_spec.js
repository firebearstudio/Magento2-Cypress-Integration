...
context('Import Products Round Price Condition 24',{ retries: 0 }, () => {
    it('round price - condition - csv - file  - new job', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - round price - condition - csv - file ')
        ...
        cy.get('[name="price_rules_rows[0][value]"]').invoke('val', '5').trigger('change',{force:true})
        ...
        cy.get('#conditions__1__children').find('span').contains('is').click()
        cy.get('[name="rule[conditions][1--1][operator]"]').select('is not',{force:true})
        ...
        cy.get('.admin__data-grid-wrap').contains('71.40',{timeout: 20000})
        cy.get('.admin__data-grid-wrap').contains('14.70',{timeout: 20000})
        cy.get('.admin__data-grid-wrap').contains('17.85',{timeout: 20000})
        cy.get('table',{timeout:40000}).contains('TST-Conf-Simp-S-Green',{timeout:40000}).parentsUntil('tbody').should('not.contain','71.40')
