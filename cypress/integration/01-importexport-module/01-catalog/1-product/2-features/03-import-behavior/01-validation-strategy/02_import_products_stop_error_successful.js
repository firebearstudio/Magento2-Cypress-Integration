...
context('Import Products Stop on Error - Successful',{ retries: 0 }, () => {
    it('import products - stop on Error - successful', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - stop on error - successful')
        ...
        cy.get('[name="sku"]').invoke('val', 'stop-error-test').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 20000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})
        cy.get('.admin__data-grid-outer-wrap').contains('5 records found',{timeout: 20000})
    })
})
