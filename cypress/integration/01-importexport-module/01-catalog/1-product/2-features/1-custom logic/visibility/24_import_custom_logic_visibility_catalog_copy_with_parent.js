...
context('Import Products Custom logic Visibility Catalog Copy With Parent 24',{ retries: 0 }, () => {
    it('custom logic - visibility - Catalog - Copy - With parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - visibility - catalog - WITH parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_visibility_catalog_with_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'visibility-six').trigger('change',{force:true})
        ...
        cy.get('table',{timeout:40000}).contains('visibility-six-S-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog')
            })
            cy.get('table',{timeout:40000}).contains('visibility-six-S-Purple').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog')
            })
            cy.get('table',{timeout:40000}).contains('visibility-six-M-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog')
            })
            cy.get('table',{timeout:40000}).contains('visibility-six-M-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Catalog')
            })
        ...
        //delete 'visibility-six' products
