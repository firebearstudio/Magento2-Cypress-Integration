...
context('Import Products Custom logic Status Disabled Copy Without Parent 31',{ retries: 0 }, () => {
    it('custom logic - status - disabled - copy - without parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - status - disabled  - Without parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_status_disabled_without_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'status-one').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        cy.get('table',{timeout:40000}).contains('status-one-S-Gray').parentsUntil('tbody')
            .within(function () {
