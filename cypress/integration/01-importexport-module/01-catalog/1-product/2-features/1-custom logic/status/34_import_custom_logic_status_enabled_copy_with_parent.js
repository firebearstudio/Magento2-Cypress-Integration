...
context('Import Products Custom logic Status Enabled Copy With Parent 34',{ retries: 0 }, () => {
    it('custom logic - status - enabled - copy - with parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - status - enabled  - With parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_status_enabled_with_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'status-four').trigger('change',{force:true})
        ...
        cy.get('table',{timeout:40000}).contains('status-four-S-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Enabled')
            })
            cy.get('table',{timeout:40000}).contains('status-four-S-Purple').parentsUntil('tbody')
            .within(function () {
                cy.contains('Enabled')
            })
            cy.get('table',{timeout:40000}).contains('status-four-M-Gray').parentsUntil('tbody')
            .within(function () {
                cy.contains('Enabled')
            })
            cy.get('table',{timeout:40000}).contains('status-four-M-Green').parentsUntil('tbody')
            .within(function () {
                cy.contains('Enabled')
            })
