...
context('Import Products Custom logic Status Disabled Copy With Parent 33',{ retries: 0 }, () => {
    it('custom logic - status - disabled - copy - with parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - status - disabled  - With parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_status_disabled_with_parent.csv')
        ...
        cy.get('[name="copy_simple_value[0][copy_simple_value_attributes]"]').select('status');
        ...
        cy.get('[name="sku"]').invoke('val', 'status-three').trigger('change',{force:true})
        ...
        cy.get('table',{timeout:40000}).contains('status-three-S-Green').parentsUntil('tbody')
            .each(function () {
                cy.contains('Disabled')
            })
            cy.get('table',{timeout:40000}).contains('status-three-S-Purple').parentsUntil('tbody')
            .each(function () {
                cy.contains('Disabled')
            })
            cy.get('table',{timeout:40000}).contains('status-three-M-Gray').parentsUntil('tbody')
            .each(function () {
                cy.contains('Disabled')
            })
            cy.get('table',{timeout:40000}).contains('status-three-M-Green').parentsUntil('tbody')
            .each(function () {
                cy.contains('Disabled')
            })
