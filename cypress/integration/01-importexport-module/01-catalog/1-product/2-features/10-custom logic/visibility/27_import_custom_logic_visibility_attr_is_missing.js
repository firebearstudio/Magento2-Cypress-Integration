...
context('Import Products Custom logic Visibility attr is missing 27', () => {
    it('custom logic - visibility attribute is missing - new job', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - visibility attr is missing')
        ...
        cy.googlePathSource('https://docs.google.com/spreadsheets/d/13FemIzzexF5koAdQYjbcKscqoCfXyknYWkQkbSZGPsk/edit#gid=1187984163')
        ...
        cy.get('.admin__data-grid-outer-wrap').contains('10 records found',{timeout: 10000})

        //check visibility values
        cy.get('table',{timeout:40000}).contains('Test Configurable-simple product-S-Gray',{timeout:40000}).parentsUntil('tbody')
        .each(function () {
            cy.contains('Not Visible Individually')
        })
        cy.get('table',{timeout:40000}).contains('NEW-S-Green').parentsUntil('tbody')
        .each(function () {
            cy.contains('Not Visible Individually')
        })
        cy.get('table',{timeout:40000}).contains('NEW-S-Purple').parentsUntil('tbody')
        .each(function () {
            cy.contains('Not Visible Individually')
        })
        cy.get('table',{timeout:40000}).contains('NEW-M-Gray').parentsUntil('tbody')
        .each(function () {
            cy.contains('Not Visible Individually')
        })
        cy.get('table',{timeout:40000}).contains('NEW-S-Purple').parentsUntil('tbody')
