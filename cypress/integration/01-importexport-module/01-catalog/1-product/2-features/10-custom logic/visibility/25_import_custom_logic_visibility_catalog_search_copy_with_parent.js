...
context('Import Products Custom logic Visibility Catalog/Search Copy With Parent 25',{ retries: 0 }, () => {
    it('custom logic - visibility - Catalog/Search - Copy - With parent', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - custom logic - copy - visibility - Catalog/Search - WITH parent')
        ...
        cy.fileSource('pub/media/importexport/test/products_custom_logic_visibility_catalog_search_with_parent.csv')
        ...
        cy.get('[name="sku"]').invoke('val', 'visibility-seven').trigger('change',{force:true})
        cy.get('[data-bind="i18n: \'Apply Filters\'"]',{timeout: 10000}).as('applyFiltersButton')
        cy.get('@applyFiltersButton').click({force:true})

        cy.get('table',{timeout:40000}).contains('visibility-seven-S-Gray').parentsUntil('tbody')
            .each(function () {
                cy.contains('Catalog, Search')
            })
        cy.get('table',{timeout:40000}).contains('visibility-seven-S-Green').parentsUntil('tbody')
            .each(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-seven-S-Purple').parentsUntil('tbody')
            .each(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-seven-M-Gray').parentsUntil('tbody')
            .each(function () {
                cy.contains('Catalog, Search')
            })
            cy.get('table',{timeout:40000}).contains('visibility-seven-M-Green').parentsUntil('tbody')
            .each(function () {
                cy.contains('Catalog, Search')
            })
        ...
        //delete 'visibility-seven' products
