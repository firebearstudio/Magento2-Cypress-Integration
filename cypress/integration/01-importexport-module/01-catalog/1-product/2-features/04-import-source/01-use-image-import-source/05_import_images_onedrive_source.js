...
context('Import Products Images One Drive Source 5', () => {
    it('import products - images - oneDrive - use image import source', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Product Import - images - oneDrive - use image import source')
        ...
        cy.get('[data-index="onedrive_onedrive_file_path"]').find('[name="onedrive_file_path"]')
            .type('test/test_images_2.csv',{force:true})
            .should('have.value', 'test/test_images_2.csv') 
        cy.get('[data-index="onedrive_onedrive_import_images_file_dir"]').find('input').as('imagesFilePath')
        cy.get('@imagesFilePath').invoke('val','test/').trigger('change',{force:true})
