...
context('Import Categories Update 2', () => {
    it(' import - update category - new job', () => {
        //login
        ...
        cy.generalImportSectionWithoutReIndex('Categories Import - update category')
        ...
        cy.fileSource('pub/media/importexport/test/category_detail_check_update.csv')
        ...
        cy.get('[data-index="content"]').find('.file-uploader').find('.file-uploader-filename').contains('improved_mport_export_magento_2_.png')
        cy.get('[name="landing_page"]').contains('Please select a static block.')
        ...
        cy.get('[name="display_mode"]').find('[value="PRODUCTS"]').should('be.selected')
        ...
        cy.get('[name="available_sort_by"]').find('[value="name"]').should('not.be.selected')
        cy.get('[name="available_sort_by"]').find('[value="price"]').should('be.selected')
        cy.get('[name="default_sort_by"]').find('[value="price"]').should('be.selected')
        cy.get('[name="filter_price_range"]').should('have.value','19.000000')
        ...
        cy.get('[data-index="url_key"]').find('[name="url_key"]').should('have.value','category-detail-check-update')
        cy.get('[data-index="meta_title"]').find('[name="meta_title"]').should('have.value','update Meta title')
        cy.get('[data-index="meta_keywords"]').find('[name="meta_keywords"]').should('have.value','update,Meta, Keywords')
        cy.get('[data-index="meta_description"]').find('[name="meta_description"]').should('have.value','update Meta Description')

        //checking values in the Design Section
        cy.get('[data-index="design"]').find('.fieldset-wrapper-title').click({force:true});
        cy.get('[data-index="custom_use_parent_settings"]').find('[name="custom_use_parent_settings"]').should('have.value','1')
        cy.get('[data-index="custom_design"]').find('[name="custom_design"]').should('have.value','3')
        cy.get('[data-index="page_layout"]').find('[name="page_layout"]').should('have.value','2columns-right')
