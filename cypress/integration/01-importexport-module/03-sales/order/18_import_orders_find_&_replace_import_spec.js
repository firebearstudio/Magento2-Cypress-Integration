...
context('Import Orders find and replace 18', () => {
    it(' find and replace - csv - file - new job', () => {
        ...
        cy.generalImportSectionWithoutReIndex('Orders Import - find and replace - csv - file')
        ...
        cy.googlePathSource('https://docs.google.com/spreadsheets/d/13FemIzzexF5koAdQYjbcKscqoCfXyknYWkQkbSZGPsk/edit#gid=1384891070')
        ...
        cy.get('[data-index="data_source_replacing_attribute"]').find('[name="[0]"]',{timeout: 60000}).select('status',{force:true});
        ...
        //Third row
        cy.get('@addNewRow').click()
        cy.get('[name="[2]"]').select('address:firstname',{force:true});
        ...
        cy.get('.order-billing-address',{timeout: 60000}).contains('Test New')
        cy.get('.order-information-table',{timeout: 60000}).contains('Pending')  
    })
})
