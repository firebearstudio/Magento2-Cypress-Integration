context('Import Shared Catalog Add Update Csv GoogleSheet 1', () => {
    it('add update - csv - google sheet - new job', () => {
        ...
        cy.get('table').contains('Shared Catalog 1').parents('tr').contains('Select').click() 
        cy.get('table').contains('Shared Catalog 1').parents('tr').find('ul').find('[data-repeat-index="2"]').click() 
        cy.get('[data-index="name"]',{timeout: 10000}).find('input').should('have.value','Shared Catalog 1')   
        ...
        cy.wait(10000)
        cy.get('table').contains('Shared Catalog 1').parents('tr').contains('Select').click() 
        cy.get('table').contains('Shared Catalog 1').parents('tr').find('ul').find('[data-repeat-index="1"]').click()
        cy.get('table').contains('Google',{timeout: 20000}) 
    })
})